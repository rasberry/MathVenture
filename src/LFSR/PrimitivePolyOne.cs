...
public static class PrimitivePolyOne
{
	public static void FindAll(ulong degree = 9)
	{
		...
		do {
			...
			sb.Append(p + new string(' ',5 - (int)System.Math.Log10(p)));
			if (pq) {
				if (nq) { sb.Append("  N"); }
				else    { sb.Append("   "); }
				if (pq) { sb.Append("  P"); }
	...
	public static void FindOne(ulong degree = 9)
	{
		var sb = new System.Text.StringBuilder();
		var ip = new all_irredpoly(degree);
	}
}
...
public class bit_necklace
{
	...
	public ulong next()
	{
		// Log.Debug($"bit_necklace a_={a_} j_={j_} n2_={n2_} j2_={j2_} n_={n_} mm_={mm_} tfb_={tfb_}");
		if (a_ == mm_) {
			first();
			return 0;
		}
